...
document.addEventListener('DOMContentLoaded', function() {
    // ----- client_mode toggle -----
    const modeRadios   = document.querySelectorAll('input[name="client_mode"]');
    ...
    function setRequired(selector, val){
        document.querySelectorAll(selector).forEach(el => el.required = !!val);
    }

    function applyMode(mode){
        [secExisting,secNew,secProspect].forEach(s => s.classList.add('hidden'));
        clientSelect.required = false;
        setRequired('[name="new_name"]', false);
        setRequired('[name="prospect_name"]', false);
        ...
        if(mode==='existing'){
            ...
            setRequired('[name="new_name"]', true);
        } else {
            secProspect.classList.remove('hidden');
            setRequired('[name="prospect_name"]', true);
        }
    }

    applyMode(document.querySelector('input[name="client_mode"]:checked')?.value || 'existing');
    modeRadios.forEach(r => r.addEventListener('change', e => applyMode(e.target.value)));
    ...
    function calculateTotals(){
        ...
        document.querySelectorAll('#itemsTable tr').forEach(row => {
            const q   = parseFloat(row.querySelector('input[name*="[quantite]"]').value) || 0;
            ...
            const rem = parseFloat(row.querySelector('input[name*="[remise]"]').value) || 0;

            let lineHT = q * pu;
            if(rem > 0) lineHT -= lineHT * rem / 100;
            const lineTVA = lineHT * (tva/100);

            row.querySelector('.total-cell').textContent = lineHT.toFixed(2) + ' €';
    ...
    document.getElementById('addFromCatalogBtn').addEventListener('click', () => document.getElementById('productCatalogModal').classList.remove('hidden'));
    document.getElementById('closeCatalogBtn').addEventListener('click', () => document.getElementById('productCatalogModal').classList.add('hidden'));
    ...
    document.getElementById('addSelectedBtn')?.addEventListener('click', () => {
        ...
        if(!selected.length){ alert('Veuillez sélectionner au moins un produit'); return; }
        selected.forEach(cb => {
            addProduitRow({
    ...
    document.getElementById('catalogSearch')?.addEventListener('input', (e) => {
        ...
        document.querySelectorAll('#catalogTable tr').forEach(tr => {
            ...
            tr.style.display = (name.includes(term) || desc.includes(term) || code.includes(term)) ? '' : 'none';
        });
    });
    ...
    document.getElementById('devisForm').addEventListener('submit', function(e){
        ...
        if(mode === 'existing' && !clientSelect.value){
            e.preventDefault(); alert('Sélectionnez un client existant ou changez de mode.'); return;
        }
        if(mode === 'new'){
            const name = document.querySelector('[name="new_name"]')?.value.trim();
            if(!name){ e.preventDefault(); alert('Renseignez au moins le nom du nouveau client.'); return; }
        }
        if(mode === 'prospect'){
            const name = document.querySelector('[name="prospect_name"]')?.value.trim();
            if(!name){ e.preventDefault(); alert('Renseignez le nom du prospect.'); return; }
        }
    });
});
</script>
